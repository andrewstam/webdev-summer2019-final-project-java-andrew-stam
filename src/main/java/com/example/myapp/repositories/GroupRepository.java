...
public interface GroupRepository extends CrudRepository<Group, Long> {
    ...
    @Query(value = "select * from group where id=:id", nativeQuery = true)
    public Group findGroupById(@Param("id") Long id);

    // Use JPA to find all groups of a movie by its id, return those groups (text only)
    @Query(value = "select leader_id from group where id=:id", nativeQuery = true)
    public List<String> findGroupLeader(@Param("id") Long id);
